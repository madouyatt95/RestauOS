        ...
        {filteredOrders.map(order => (
          <motion.div layout key={order.id} className="glass-card p-4 border-white/5 flex items-center justify-between active:scale-98 transition-transform">
            <div className="flex items-center gap-4">
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 ${order.status === 'paye' ? 'bg-green/10 text-green' : 'bg-orange/10 text-orange'}`}>
                <FileText size={20} />
              </div>
              <div>
            ...
            <div className="text-right">
              ...
              <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${order.status === 'paye' ? 'bg-green/10 text-green' : 'bg-orange/10 text-orange'}`}>
                {order.status}
              </span>
            </div>
          </motion.div>
